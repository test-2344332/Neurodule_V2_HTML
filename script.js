...
//		this.ctx.fillRect(0,0,this.width,this.height);
	...
	sort_content() {
		if (this.contents != this.old_cont) {
			let new_cont = [];
			let l = this.layers;
			for (let i = 0; i < l; i++) {
...
class Element {
	...
	change_position(x, y, d = 0) {
		...
		if (temp_x > this.get_display_val("width")) {
			...
			console.log(
				"Tried to set x out of bounds replacing with maximum value allowed",
			);
		} else if (temp_x < 0) {
			temp_x = 0;
			console.log(
		...
		if (temp_y > this.get_display_val("height")) {
			...
			console.log(
				"Tried to set y out of bounds replacing with maximum value allowed",
			);
		} else if (temp_y < 0) {
			temp_y = 0;
			console.log(
		...
		if (temp_d > this.#layers) {
			temp_d = this.#layers;
			console.log(
				"Tried to set depth out of bounds replacing with maximum value allowed",
			);
		} else if (temp_d < 0) {
			temp_d = 0;
			console.log(
